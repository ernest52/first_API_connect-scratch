...
app.get("/actors/:fullName", async (req, res) => {
  try {
    const response = await axios.get(
      `https://api.tvmaze.com/search/people?q=${req.params.fullName}`
    );

    const index = actors.findIndex((actor) => actor.id === req.params.fullName);

    const person = response.data[0].person;

    const { url: more, name, country, birthday, deathday, image } = person;
    const actor = {
      more,
      name,
      country,
    ...
    res.json({ actor, actors, index });
  } catch (err) {
    console.log("Error: ", err);
  }
});

app.get("/series/:genres", async (req, res) => {
  ...
  try {
    ...
    let cur = 0;
    let i = 0;
    let max = response.data.length < 8 ? response.data.length : 8;

    while (i < max) {
      const img = response.data[i].show.image;
      // const text = response.data[i].show.summary;
      if (img) {
        const {
          id,
          name,
...
app.get("/serie/:id", async (req, res) => {
  try {
    ...
    const { name, url: more, image, summary: text } = response.data;
